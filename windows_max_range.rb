...
Learning Goals
Practice approaching a difficult problem using algorithms
...
What if it were possible to calculate min and max instantaneously (in constant time) per window? This would allow us to find the windowed max range in O(n) time. We can achieve this by writing a custom data structure dedicated to solving this specific problem.
...
A queue is a simple abstract linear data structure where elements are stored in order and can be added or removed one at a time. A queue follows the first in, first out (FIFO) principle. Unlike Ruby's Array data structure, most Queue implementations do not expose methods to slice or sort the data, or to find a specific element. The basic operations are:
...
Queues may be implemented in terms of simpler data structures, such as linked lists, but in Ruby you can actually use an Array as the underlying data store, as long as you don't expose it publicly (i.e., don't define an attr_reader for it). We'll use an Array in today's exercises.
...
Code Review: Request a TA code review at the end of this phase.
...
Hint: We could store some metadata with the value of each element. In other words, we can store more information than just the value with each new element added to the store. Think about how to do this and what information to store.

Implement peek, size, empty?, max, min, pop, and push methods on your MinMaxStack.

Phase 6: MinMaxStackQueue
Just as we used our MyStack (phase 3) to build our StackQueue (phase 4), use your MinMaxStack (phase 5) to build a MinMaxStackQueue.

What methods are needed to solve this problem in O(n) time?

Phase 7: Windowed Max Range
Now that you have a working MinMaxStackQueue, windowed max range should be a much easier problem to solve. You'll want to follow the same basic approach as above, but use our new data structure instead of array slices. As before, return the current_max_range at the end of the method. Make sure all the test cases pass, and that both of you understand the time complexity of this solution.
